...
public class ClientModel {
	...
	/*
	 * public void add(ClientBean bean) throws Exception { // Establishing a
	 * connection try (Connection conn = JDBCDataSource.getConnection();
	 * PreparedStatement pstmt =
	 * conn.prepareStatement("INSERT INTO st_client VALUES (?,?,?,?,?,?,?,?,?)")) {
	 * 
	 * // Fetching customer details ClientModel model = new ClientModel();
	 * CustomerBean customerBean = customerModel.findByPk(bean.getCustomerId());
	 * 
	 * if (customerBean == null) { throw new Exception("Customer with ID " +
	 * bean.getCustomerId() + " not found."); }
	 * 
	 * // Setting customer name bean.setCustomerName(customerBean.getFirstName() +
	 * " " + customerBean.getLastName());
	 * 
	 * // Getting the next primary key int pk = nextPk();
	 * 
	 * // Setting parameters for the prepared statement pstmt.setLong(1, pk);
	 * pstmt.setLong(2, bean.getCustomerId()); pstmt.setString(3,
	 * bean.getCustomerName()); pstmt.setDate(4, new
	 * java.sql.Date(bean.getOrderDate().getTime())); pstmt.setLong(5,
	 * bean.getTotalAmount()); pstmt.setString(6, bean.getCreatedBy());
	 * pstmt.setString(7, bean.getModifiedBy()); pstmt.setTimestamp(8,
	 * bean.getCreatedDatetime()); pstmt.setTimestamp(9,
	 * bean.getModifiedDatetime());
	 * 
	 * // Executing the update int i = pstmt.executeUpdate();
	 * 
	 * if (i > 0) { System.out.println("Data inserted successfully! Rows affected: "
	 * + i); } else { throw new Exception("Data insertion failed."); } } catch
	 * (Exception e) { // Handle exceptions e.printStackTrace(); throw e; // Rethrow
	 * the exception for higher-level handling } }
	 */
	...
	public void update(OrderBean bean) throws Exception {
		...
		PreparedStatement pstmt = conn.prepareStatement(
				"update st_client set customer_id = ?, customer_name = ?, order_date = ?, total_amount = ?, created_by = ?, modified_by = ?, created_datetime = ?, modified_datetime = ? where id = ?");
	...
	public OrderBean findByTotalAmount(String totalAmount) throws Exception {
		...
		PreparedStatement pstmt = conn.prepareStatement("select * from st_client where total_amount = ?");
