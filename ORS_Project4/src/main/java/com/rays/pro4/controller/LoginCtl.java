	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			try {
				...
				if (bean != null) {
					session.setAttribute("user", bean);/// bean ko user mai hold kiya
					long rollId = bean.getRoleId();/// bean.getgetroleid ko long rollid mai hold kiya

					RoleBean rolebean = role.findByPK(rollId);//// rolemodel ki find bu pk main rollid di aur rolebean
																//// mai set kiya
